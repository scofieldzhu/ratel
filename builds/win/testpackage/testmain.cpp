...
void TestCase_PrintTree(const DirNode& rootnode, int32& layer)
{
    ...
    for (const DirNode* childnode = &rootnode; childnode != nullptr; childnode = childnode->next_sibling) {
        cout << "Dir:" << childnode->dirname.cstr() << " ";
        for (const FileNode& fn : childnode->allfiles)
            cout << "File:" << fn.filename.cstr() << " ";
        cout << endl;
        if (nextlayer_child == nullptr && childnode->next_child)
            nextlayer_child = childnode->next_child;
    }
    if (nextlayer_child)
